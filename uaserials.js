 ...
 *
 *  НАЛАШТУВАННЯ (в самій Лампі):
 *    Налаштування → UASerials:
 *      • CORS-проксі — проміжний сервіс для доступу до сайту (порожньо = без проксі);
 *      • Адреса сайту — якщо зміниться домен.
 *
 *  ВАЖЛИВО:
 *    - Реєстрація в Лампі, кнопка, налаштування, обробка помилок — перевірені.
 *    - Частина ПАРСИНГУ сайту (як читаються результати й відео) може
 *      потребувати підлаштування під поточну верстку — місця позначені
...
(function () {
    ...
    // Значення беруться з налаштувань Лампи (розділ "UASerials"),
    // а якщо там порожньо — з CONFIG за замовчуванням.
    function getCors() {
        return Lampa.Storage.get('uaserials_cors', CONFIG.cors);
    }
    function getSite() {
        return (Lampa.Storage.get('uaserials_site', CONFIG.site) || CONFIG.site).replace(/\/+$/, '');
    }

    function proxied(url) {
        var cors = getCors();
        if (!cors) return url;
        if (cors.indexOf('url=') !== -1) return cors + encodeURIComponent(url);
        return cors + url;
    }
    ...
    function absolute(href) {
        ...
        if (href.indexOf('/') === 0) return getSite() + href;
        return getSite() + '/' + href;
    }
    ...
    var Source = {
        ...
        search: function (query, onDone, onError) {
            var _this = this;
            var url = getSite() + '/index.php?do=search&subaction=search&story=' + encodeURIComponent(query);
    ...
    function Component(object) {
        ...
        this.fetch = function () {
            ...
            if (!query) {
                this.empty('Порожня назва', 'Картка не передала назву для пошуку.');
                return;
            }
            ...
            Source.search(query, function (results) {
                ...
                if (!results.length) {
                    _this.empty(
                        'Нічого не знайдено: ' + query,
                        'Запит пройшов, але жодного результату. Найімовірніше не збіглися селектори ' +
                        'у функції parseSearch (розмітка сайту). Місце позначене "НАЛАШТУВАТИ ПІД САЙТ".'
                    );
                    return;
                }
                _this.draw(results);
                ...
                _this.empty(
                    'Помилка запиту',
                    err + '<br><br>Найчастіша причина — CORS-проксі. Зміни його в ' +
                    'Налаштування → UASerials → CORS-проксі (або залиш поле порожнім).'
                );
            });
        };
        ...
        // Показує причину прямо на екрані (замість порожнього шаблону {назва}/{опис}).
        this.empty = function (text, hint) {
            html.empty().append($(
                '<div style="padding:2.5em 2em;text-align:center;max-width:40em;margin:0 auto;">' +
                    '<div style="font-size:1.4em;font-weight:600;margin-bottom:.6em;">' +
                        (text || 'Порожньо') +
                    '</div>' +
                    (hint ? '<div style="opacity:.6;line-height:1.5;">' + hint + '</div>' : '') +
                '</div>'
            ));
            Lampa.Controller.enable('content');
        };
    ...
    // ------------------------------------------------------------------
    // 6. НАЛАШТУВАННЯ ПЛАГІНА (Налаштування → UASerials)
    // ------------------------------------------------------------------
    ...
    function addSettings() {
        ...
        Lampa.SettingsApi.addComponent({
            component: CONFIG.component,
            name: CONFIG.title,
            icon: '<svg height="24" viewBox="0 0 24 24" width="24" fill="none" xmlns="http://www.w3.org/2000/svg">' +
                  '<circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="2"/>' +
                  '<path d="M10 8.5l5 3.5-5 3.5z" fill="currentColor"/></svg>'
        });

        // Поле: CORS-проксі (вводиться вручну; порожньо = без проксі)
        Lampa.SettingsApi.addParam({
            component: CONFIG.component,
            param: { name: 'uaserials_cors', type: 'input', values: '', default: CONFIG.cors },
            field: {
                name: 'CORS-проксі',
                description: 'Проміжний сервіс, що підставляється перед адресою сайту. Порожньо = без проксі.'
            },
            onChange: function () { Lampa.Settings.update(); }
        });

        // Поле: адреса сайту (на випадок зміни домену)
        Lampa.SettingsApi.addParam({
            component: CONFIG.component,
            param: { name: 'uaserials_site', type: 'input', values: '', default: CONFIG.site },
            field: {
                name: 'Адреса сайту',
                description: 'Домен джерела, напр. https://uaserials.com (без / в кінці).'
            },
            onChange: function () { Lampa.Settings.update(); }
        });
    }
